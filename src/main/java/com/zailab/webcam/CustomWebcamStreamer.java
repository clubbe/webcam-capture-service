...
public class CustomWebcamStreamer implements ThreadFactory, WebcamListener {

	private static final Logger LOG = LoggerFactory.getLogger(CustomWebcamStreamer.class);
		...
		public void run() {
			try {
				...
				while (started.get()) {
					...
					LOG.info("New connection from {} on {}", socket.getRemoteSocketAddress(), server);
		...
		public void run() {
			...
			try {
				while (br.ready()) {
					...
					LOG.info("Read input from {} = {}", socket.getRemoteSocketAddress(), readLine);
				}
			} catch (IOException e) {
			...
			try {
				...
				while (started.get()) {
					...
					do {
						...
						try {
							...
							// LOG.error("Socket exception from " + socket.getRemoteSocketAddress(), e); TODO
							br.close();
							bos.close();
							return;
						}
				...
				for (Closeable closeable : new Closeable[] { br, bos, baos }) {
					try {
						...
						// LOG.error("Cannot close socket", e); TODO
					}
				}
				try {
					socket.close();
					LOG.info("Closed connection from {}", socket.getRemoteSocketAddress());
				} catch (IOException e) {
	...
	public CustomWebcamStreamer(int port, Webcam webcam, double fps, boolean start) {
